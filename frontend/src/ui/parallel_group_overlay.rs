...
fn find_branch_nodes(
    ...
    branch_node_ids.sort_by(|a, b| {
        ...
        match (node_a, node_b) {
            (Some(na), Some(nb)) => {
                (na.x, na.y).partial_cmp(&(nb.x, nb.y)).unwrap_or(std::cmp::Ordering::Equal)
            }
...
fn calculate_bounding_box(branch_nodes: &[Node]) -> Option<BoundingBox> {
    ...
    let min_x = branch_nodes.iter().map(|n| n.x).fold(f32::INFINITY, f32::min);
    let max_x = branch_nodes.iter().map(|n| n.x + NODE_WIDTH).fold(f32::NEG_INFINITY, f32::max);
    let min_y = branch_nodes.iter().map(|n| n.y).fold(f32::INFINITY, f32::min);
    let max_y = branch_nodes.iter().map(|n| n.y + NODE_HEIGHT).fold(f32::NEG_INFINITY, f32::max);
