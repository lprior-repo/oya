...
fn parse_http_method(config: &Value, key: &str) -> HttpMethod {
    get_str_val(config, key).parse::<HttpMethod>().unwrap_or_default()
}
...
fn durable_config(icon: &str, config: &Value, on_change: EventHandler<Value>) -> Element {
    let step_field = text_field("Step Name", "durableStepName", config, "create-user", on_change);
    ...
    if icon == "clock-send" {
        ...
        let target_field =
            text_field("Target", "targetService", config, "PaymentService", on_change);
        rsx! {
            {step_field}
            {target_field}
        }
...
fn flow_config(icon: &str, config: &Value, on_change: EventHandler<Value>) -> Element {
    match icon {
        ...
        "undo" => {
            text_field("Compensation", "compensationHandler", config, "refundPayment", on_change)
        }
        _ => rsx! { div { class: "text-[10px] italic text-slate-400", "No quick config" } },
    }
}
...
fn signal_config(icon: &str, config: &Value, on_change: EventHandler<Value>) -> Element {
    match icon {
        "target" | "check-circle" => {
            text_field("Promise Name", "promise_name", config, "payment-completed", on_change)
        }
        "radio" => {
            text_field("Awakeable ID", "awakeable_id", config, "payment-callback", on_change)
        }
        "bell" => text_field("Signal Name", "signal_name", config, "payment_signal", on_change),
        _ => rsx! { div { class: "text-[10px] italic text-slate-400", "No quick config" } },
    }
}
