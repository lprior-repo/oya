...
const PALETTE_ENTRIES: [PaletteEntry; 9] = [
    PaletteEntry { node_type: NodeTemplateId::HttpHandler, icon: "🌐" },
    PaletteEntry { node_type: NodeTemplateId::Run, icon: "🛡️" },
    PaletteEntry { node_type: NodeTemplateId::Sleep, icon: "⏱️" },
    PaletteEntry { node_type: NodeTemplateId::SetState, icon: "⬆️" },
    PaletteEntry { node_type: NodeTemplateId::GetState, icon: "⬇️" },
    PaletteEntry { node_type: NodeTemplateId::SendMessage, icon: "📤" },
    PaletteEntry { node_type: NodeTemplateId::Condition, icon: "⑂" },
    PaletteEntry { node_type: NodeTemplateId::Parallel, icon: "⫿" },
    PaletteEntry { node_type: NodeTemplateId::Condition, icon: "⑂" },
];
...
mod tests {
    ...
    fn node(node_type: NodeTemplateId) -> SketchNode {
        SketchNode { node_type, label: node_type.label().to_string() }
    }
    ...
    fn given_three_nodes_when_generating_skeleton_then_linear_chain_is_correct() {
        ...
        let step2_block: Vec<&str> = lines
            ...
            .copied()
            .collect();
        assert!(step2_block.iter().any(|l| l.contains("depends_on: [step-1]")));

        let step3_block: Vec<&str> =
            lines.iter().skip_while(|l| !l.contains("id: step-3")).copied().collect();
        assert!(step3_block.iter().any(|l| l.contains("depends_on: [step-2]")));
