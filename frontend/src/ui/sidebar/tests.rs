...
fn given_all_categories_when_getting_icon_badge_classes_then_none_are_empty() {
    for cat in Category::ORDER {
        assert!(!cat.icon_badge_class().is_empty(), "{cat:?} has an empty icon_badge_class");
    }
}
...
fn given_schedule_query_when_matching_templates_then_cron_trigger_is_found() {
    let template = NODE_TEMPLATES.iter().find(|t| t.node_type == "cron-trigger");

    assert!(template.is_some(), "cron-trigger must exist in NODE_TEMPLATES");
    assert!(
...
fn given_friendly_phrase_when_matching_templates_then_send_message_matches() {
    let template = NODE_TEMPLATES.iter().find(|t| t.node_type == "send-message");

    assert!(template.is_some(), "send-message must exist in NODE_TEMPLATES");
    assert!(
...
fn given_unrelated_query_when_matching_http_handler_then_no_match() {
    let template = NODE_TEMPLATES.iter().find(|t| t.node_type == "http-handler");

    assert!(template.is_some(), "http-handler must exist in NODE_TEMPLATES");
    assert!(
...
fn given_empty_query_when_matching_any_template_then_always_matches() {
    for template in &NODE_TEMPLATES {
        assert!(template.matches_query(""), "{} should match an empty query", template.node_type);
    }
}
...
fn given_entry_category_with_empty_query_when_getting_indices_then_all_entry_templates_returned() {
    ...
    let expected_count = NODE_TEMPLATES.iter().filter(|t| t.category == Category::Entry).count();
...
fn given_node_templates_when_checking_types_then_all_node_types_are_non_empty() {
    for t in &NODE_TEMPLATES {
        ...
        assert!(!t.label.is_empty(), "label must not be empty for {}", t.node_type);
        assert!(!t.icon.is_empty(), "icon must not be empty for {}", t.node_type);
    }
}
...
fn given_category_order_when_iterating_then_all_six_categories_are_represented() {
    ...
    for cat in Category::ORDER {
        assert!(represented.contains(&cat), "No template found for category {cat:?}");
    }
}
