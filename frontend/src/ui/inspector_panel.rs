...
pub fn InspectorPanel(
    ...
    let start_str = step_start_time.read().clone().unwrap_or_else(|| "—".to_string());
    let end_str = step_end_time.read().clone().unwrap_or_else(|| "—".to_string());
    ...
    let input_json_text =
        step_input.read().as_ref().map_or_else(|| "null".to_string(), pretty_json);

    let output_json_text =
        step_output.read().as_ref().map_or_else(|| "null".to_string(), pretty_json);
...
mod tests {
    ...
    fn given_idle_state_when_getting_badge_class_then_returns_slate() {
        ...
        assert!(class.contains("slate"), "expected slate class, got: {class}");
    }
    ...
    fn given_completed_state_when_getting_badge_class_then_returns_green() {
        ...
        assert!(class.contains("green"), "expected green class, got: {class}");
    }
