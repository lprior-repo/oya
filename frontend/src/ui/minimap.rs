...
fn scene_bounds(nodes: &[Node]) -> SceneBounds {
    nodes
        ...
        .map_or(
            SceneBounds { min_x: -200.0, min_y: -200.0, width: 1200.0, height: 1000.0 },
...
fn node_colors(category: &str, selected: bool) -> NodeColors {
    if selected {
        return NodeColors { fill: "rgba(99,102,241,0.40)", stroke: "rgba(129,140,248,0.90)" };
    }
    match category {
        "entry" => NodeColors { fill: "rgba(96,165,250,0.25)", stroke: "rgba(96,165,250,0.70)" },
        "durable" => NodeColors { fill: "rgba(74,222,128,0.25)", stroke: "rgba(74,222,128,0.70)" },
        "state" => NodeColors { fill: "rgba(34,211,238,0.25)", stroke: "rgba(34,211,238,0.70)" },
        "flow" => NodeColors { fill: "rgba(244,114,182,0.25)", stroke: "rgba(244,114,182,0.70)" },
        "timing" => NodeColors { fill: "rgba(192,132,252,0.25)", stroke: "rgba(192,132,252,0.70)" },
        "signal" => NodeColors { fill: "rgba(251,191,36,0.25)", stroke: "rgba(251,191,36,0.70)" },
        _ => NodeColors { fill: "rgba(100,116,139,0.25)", stroke: "rgba(100,116,139,0.70)" },
    }
}
...
mod tests {
    ...
    fn given_valid_viewport_when_projecting_rect_then_math_is_correct() {
        let vp = Viewport { x: 200.0, y: 100.0, zoom: 2.0 };
    ...
    fn given_min_clamped_zoom_when_projecting_rect_then_zoom_is_valid() {
        let vp = Viewport { x: 0.0, y: 0.0, zoom: 0.0_f32.clamp(0.15, 3.0) };
        let rect = viewport_rect(&vp, 800.0, 600.0);
