...
impl PayloadShape {
    ...
    pub fn from_value(value: &serde_json::Value) -> Self {
        match value {
            serde_json::Value::Object(map) => Self::Object { field_count: map.len() },
            serde_json::Value::Array(arr) => Self::Array { element_count: arr.len() },
            serde_json::Value::String(s) => Self::String { char_count: s.len() },
...
pub const fn invocation_badge_style(status: InvocationStatus) -> StatusBadgeStyle {
    match status {
        ...
        InvocationStatus::Running => {
            StatusBadgeStyle::new("bg-indigo-500/15", "text-indigo-400", "border-indigo-500/30")
        }
        InvocationStatus::Suspended => {
            StatusBadgeStyle::new("bg-pink-500/15", "text-pink-400", "border-pink-500/30")
        }
        InvocationStatus::Completed => {
            StatusBadgeStyle::new("bg-emerald-500/15", "text-emerald-400", "border-emerald-500/30")
        }
...
mod tests {
    ...
    fn invocation_status_parse_handles_variants() {
        assert_eq!(InvocationStatus::parse("pending"), Some(InvocationStatus::Pending));
    ...
    fn validation_result_category_from_counts() {
        ...
        assert_eq!(ValidationResultCategory::from_counts(0, 0), ValidationResultCategory::Valid);
    }
}
