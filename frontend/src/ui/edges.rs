...
fn get_source_point(node: &Node) -> Position {
    Position { x: node.x + NODE_WIDTH, y: node.y + NODE_HEIGHT / 2.0 }
}

fn get_target_point(node: &Node) -> Position {
    Position { x: node.x, y: node.y + NODE_HEIGHT / 2.0 }
}
...
fn create_smooth_step_path(from: Position, to: Position, bend_y: f32) -> (String, Position) {
    ...
    let midpoint = Position { x: f32::midpoint(from.x, to.x), y: mid_y };
    ...
    edges
        ...
        .filter_map(|edge| {
            ...
            let adjusted_to = group.map_or(to, |g| {
                let branch_nodes: Vec<Node> =
                    g.branch_node_ids.iter().filter_map(|id| node_by_id.get(id).cloned()).collect();
                let offset = calculate_parallel_offset(&edge.target, &branch_nodes, NODE_HEIGHT);
                Position { x: to.x, y: to.y + offset }
            });

            Some((edge.id.to_string(), EdgeAnchor { from, to: adjusted_to }))
        })
        .collect()
}
...
fn calculate_parallel_offset(target_id: &NodeId, targets: &[Node], node_height: f32) -> f32 {
    ...
    let idx = sorted.iter().position(|(_, n)| n.id == *target_id).unwrap_or(0);
...
fn find_parallel_branches(nodes: &[Node], connections: &[Connection]) -> Vec<ParallelGroup> {
    ...
    for connection in connections {
        ...
        if parallel_node_ids.contains(&connection.source) {
            source_targets.entry(connection.source).or_default().insert(connection.target);
        }
    }
    ...
    source_targets
        ...
        .filter_map(|(source_id, target_ids)| {
            ...
            let mut target_nodes: Vec<Node> =
                target_ids.iter().copied().filter_map(|id| node_by_id.get(&id).cloned()).collect();

            target_nodes.sort_by_key(|a| a.id.0);

            let min_y = target_nodes.iter().map(|n| n.y).fold(f32::INFINITY, f32::min);
            let max_y =
                target_nodes.iter().map(|n| n.y + NODE_HEIGHT).fold(f32::NEG_INFINITY, f32::max);
            let min_x = target_nodes.iter().map(|n| n.x).fold(f32::INFINITY, f32::min);
            let max_x =
                target_nodes.iter().map(|n| n.x + NODE_WIDTH).fold(f32::NEG_INFINITY, f32::max);
...
mod tests {
    ...
    fn given_two_targets_when_calculate_offset_then_returns_symmetric_values() {
        ...
        let expected_a = if target_a_id == sorted_ids[0] { -spacing / 2.0 } else { spacing / 2.0 };
    ...
    fn given_parallel_groups_when_resolve_anchors_then_offsets_applied_to_targets() {
        ...
        let group = ParallelGroup {
            ...
            bounding_box: BoundingBox { x: 292.0, y: 92.0, width: 16.0, height: 116.0 },
            branch_count: 2,
            aggregate_status: AggregateStatus::Pending,
        };
        ...
        let expected_offset_a =
            if target_a_id == sorted_ids[0] { -spacing / 2.0 } else { spacing / 2.0 };
    ...
    fn given_shared_target_across_sources_when_resolve_anchors_then_uses_source_target_match() {
        ...
        let group_a = ParallelGroup {
            ...
            bounding_box: BoundingBox { x: 292.0, y: 192.0, width: 236.0, height: 84.0 },
            branch_count: 1,
            aggregate_status: AggregateStatus::Pending,
        };
        let group_b = ParallelGroup {
            ...
            branch_node_ids: vec![shared_target_id],
            bounding_box: BoundingBox { x: 292.0, y: 392.0, width: 236.0, height: 84.0 },
            branch_count: 1,
            aggregate_status: AggregateStatus::Pending,
        };
...
pub fn FlowEdges(
    ...
    let node_by_id = use_memo(move || {
        nodes.read().iter().cloned().map(|node| (node.id, node)).collect::<HashMap<_, _>>()
    });
    ...
    let svg_pointer_class =
        if drag_state.read().is_some() { "pointer-events-auto" } else { "pointer-events-none" };
