...
fn record_suggestion_decision(key: &str, accepted: bool, source: &str) {
    ...
    let decision =
        if accepted { SuggestionDecision::Accepted } else { SuggestionDecision::Rejected };
...
fn push_timeline(
    ...
    let mut new_timeline = vec![ExtensionTimelineEvent { id: next_id, kind, message, metadata }];
...
fn event_appearance(
    ...
    match kind {
        ...
        ExtensionTimelineEventKind::Applied => {
            ("bg-emerald-500", "bg-emerald-100 text-emerald-700", "Applied")
        }
...
fn remember_extension_snapshot(
    ...
    (
        new_snapshots,
        ExtensionTimelineMetadata { batch_id: next_batch_id, snapshot_id: next_snapshot_id, mode },
    )
}
...
fn snapshot_by_id(
    ...
    snapshots.iter().find(|entry| entry.snapshot_id == snapshot_id).cloned()
}
...
mod tests {
    ...
    fn collect_previews_ignores_unknown_keys_but_keeps_valid_previews() {
        let mut workflow = Workflow::new();
        workflow.add_node("run", 10.0, 10.0);
        let keys = vec!["unknown-extension-key".to_string(), "add-timeout-guard".to_string()];
