...
pub fn ExpressionInput(
    ...
    let new_phase = match &trigger_info {
        Some(Trigger::NodeStart) => CompletionPhase::NodeName { prefix: String::new() },
        Some(Trigger::NodePrefix(prefix)) => CompletionPhase::NodeName { prefix: prefix.clone() },
        Some(Trigger::PathStart { node_name }) => {
            CompletionPhase::Path { node_name: node_name.clone(), path_prefix: String::new() }
        }
        Some(Trigger::PathPrefix { node_name, path }) => {
            CompletionPhase::Path { node_name: node_name.clone(), path_prefix: path.clone() }
        }
    ...
    let suggestions: Vec<Suggestion> = match &new_phase {
        ...
        CompletionPhase::NodeName { prefix } => nodes
            ...
            .collect(),
        CompletionPhase::Path { node_name, path_prefix } => nodes
            .iter()
...
enum CompletionPhase {
    ...
    Path { node_name: String, path_prefix: String },
}
    ...
    if let Some(start) = value.rfind(node_pattern) {
        ...
        if let Some(quote_pos) = after_trigger.find("\"]") {
            ...
            if after_bracket.is_empty() {
                return Some(Trigger::PathStart { node_name: node_name.to_string() });
...
fn collect_paths(value: &Value, current_path: &str, prefix: &str, paths: &mut Vec<Suggestion>) {
    if let Value::Object(map) = value {
        for (key, child) in map {
            let full_path =
                if current_path.is_empty() { key.clone() } else { format!("{current_path}.{key}") };
...
mod tests {
    ...
    fn given_node_suggestion_when_applying_then_incomplete_node_token_is_replaced() {
        let suggestion =
            Suggestion::Node(super::NodeInfo { name: "Fetcher".to_string(), last_output: None });
