...
fn build_plan_snapshot(workflow: &Workflow) -> PlanSnapshot {
    ...
    let mut outgoing: HashMap<NodeId, Vec<NodeId>> =
        node_ids.iter().map(|id| (*id, Vec::<NodeId>::new())).collect();
    ...
    PlanSnapshot { layers, unscheduled }
}
...
fn QueueItem(
    ...
    let active_class = if is_current { "ring-1 ring-indigo-300 bg-indigo-50" } else { "" };
