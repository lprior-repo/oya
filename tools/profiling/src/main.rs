...
fn run() -> oya_profiling::Result<()> {
    ...
    let command = args
        .get(1)
        .ok_or_else(|| oya_profiling::ProfilingError::Config("Missing command argument".to_string()))?
        .clone();
    let command_args = args
        .get(2..)
        .ok_or_else(|| oya_profiling::ProfilingError::Config("Missing command arguments".to_string()))?
        .to_vec();
