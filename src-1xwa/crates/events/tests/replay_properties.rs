...
proptest! {
    ...
    fn prop_checkpoint_resume_equivalence(
        ...
        _checkpoint_pos in 0usize..100usize,
    ) {
        ...
        for event in &events {
            ...
            let rt = tokio::runtime::Runtime::new()
                .map_err(|e| TestCaseError::fail(format!("runtime creation should succeed: {}", e)))?;
            rt.block_on(async {
                store_full.append(event.clone()).await
            }).map_err(|e| TestCaseError::fail(format!("append should succeed: {}", e)))?;
        }
        ...
        }).map_err(|e| TestCaseError::fail(format!("full replay should succeed: {}", e)))?;
        ...
        }).map_err(|e| TestCaseError::fail(format!("checkpoint replay should succeed: {}", e)))?;
        ...
        for (id, bead_full) in &full_state.beads {
            let bead_checkpoint = checkpoint_state.beads.get(id)
                .ok_or_else(|| TestCaseError::fail(format!("Checkpoint state should contain bead {:?}", id)))?;
...
proptest! {
    ...
    fn prop_replay_determinism(
        ...
        for _ in 0..replay_count {
            ...
            }).map_err(|e| TestCaseError::fail(format!("replay should succeed: {}", e)))?;
            states.push(state);
        }

        // Property: All replays produce identical state
        let first = states.first()
            .ok_or_else(|| TestCaseError::fail("states should not be empty"))?;
        for (i, state) in states.iter().enumerate().skip(1) {
            ...
            for (id, bead_first) in &first.beads {
                let bead_other = state.beads.get(id)
                    .ok_or_else(|| TestCaseError::fail(format!("Replay {} should contain bead {:?}", i, id)))?;
...
proptest! {
    ...
    fn prop_independent_event_ordering(
        ...
        }).map_err(|e| TestCaseError::fail(format!("rebuild store1 should succeed: {}", e)))?;
        ...
        }).map_err(|e| TestCaseError::fail(format!("rebuild store2 should succeed: {}", e)))?;
        ...
        let bead1_state1 = state1.beads.get(&bead1)
            .ok_or_else(|| TestCaseError::fail("bead1 should be in state1"))?;
        let bead1_state2 = state2.beads.get(&bead1)
            .ok_or_else(|| TestCaseError::fail("bead1 should be in state2"))?;
        prop_assert_eq!(
        ...
        let bead2_state1 = state1.beads.get(&bead2)
            .ok_or_else(|| TestCaseError::fail("bead2 should be in state1"))?;
        let bead2_state2 = state2.beads.get(&bead2)
            .ok_or_else(|| TestCaseError::fail("bead2 should be in state2"))?;
        prop_assert_eq!(
...
proptest! {
    ...
    fn prop_state_preserves_event_information(
        ...
        let stored_count = rt.block_on(async {
            store.count().await
        }).map_err(|e| TestCaseError::fail(format!("count should succeed: {}", e)))?;
        ...
        let state = rt.block_on(async {
            let projection = AllBeadsProjection::new();
            projection.rebuild(&store).await
        }).map_err(|e| TestCaseError::fail(format!("rebuild should succeed: {}", e)))?;
        ...
        let bead = state.beads.get(&bead_id)
            .ok_or_else(|| TestCaseError::fail("bead should be in state"))?;
        prop_assert!(
            !bead.history.is_empty() || event_count == 1,
...
proptest! {
    ...
    fn prop_empty_and_minimal_sequences(
        ...
        let state = result.map_err(|e| TestCaseError::fail(format!("state should be Ok: {}", e)))?;
    ...
    for i in 1..count {
        match i % 5 {
            ...
            3 => events.push(BeadEvent::phase_completed(
                ...
                format!("phase_{}", i),
                oya_events::PhaseOutput::success(b"test".to_vec()),
            )),
            ...
            _ => {} // Covered by exhaustive match above
        }
    }
...
mod utility_tests {
    ...
    fn test_create_event_sequence_determinism() {
        let bead_id = BeadId::new();
        let spec1 = BeadSpec::new("Test").with_complexity(Complexity::Simple);
        let spec2 = BeadSpec::new("Test").with_complexity(Complexity::Simple);

        let seq1 = create_event_sequence(bead_id, spec1, 10);
        let seq2 = create_event_sequence(bead_id, spec2, 10);
    ...
    fn test_create_event_sequence_count() {
        ...
        let events = create_event_sequence(bead_id, spec.clone(), 5);
