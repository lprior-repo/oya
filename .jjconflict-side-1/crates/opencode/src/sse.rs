...
mod tests {
    ...
    fn test_format_message_thinking() {
        let formatter = SseFormatter::new();
        let output = Output::Thinking("analyzing the prompt".to_string());

        let message = formatter.format_message(output);
        assert!(message.contains("event: thinking"));
        assert!(message.contains("data: analyzing the prompt"));
    }
    ...
    #[test]
    fn test_format_chunks_iterator() {
        let formatter = SseFormatter::new();
        let chunks = vec![
        ...
        let message_count = messages.unwrap();
        assert_eq!(message_count.len(), 3);
    }

    #[test]
    fn test_format_chunk_error_handling() {
        let formatter = SseFormatter::new();
