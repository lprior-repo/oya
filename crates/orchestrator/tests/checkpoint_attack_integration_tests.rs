...
use orchestrator::persistence::{CheckpointRecord, OrchestratorStore, StoreConfig};
use orchestrator::replay::{CheckpointConfig, CheckpointManager};
    ...
    match result {
        ...
        Err(e) => {
                ...
                || error_msg.contains("not found")
            {
                ...
                println!(
                    "🟡 WARNING: Error message may not be actionable: {}",
                    error_msg
                );
            }
        }
...
async fn attack_malformed_checkpoint_ids() {
    ...
    for (id, description) in malformed_ids {
        ...
        match result {
            Ok(_) => {
                println!(
                    "🔴 CRITICAL: Malformed checkpoint ID '{}' ({}) succeeded!",
                    id, description
                );
            ...
            Err(e) => {
                println!(
                    "✅ GOOD: Malformed checkpoint ID '{}' ({}) rejected",
                    id, description
                );
            }
        }
...
async fn attack_corrupted_json_in_checkpoint() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            if error_msg.contains("serialization")
                ...
                || error_msg.contains("json")
            {
                ...
                println!(
                    "🟡 WARNING: Error doesn't clearly indicate JSON issue: {}",
                    error_msg
                );
            }
        }
...
async fn attack_error_message_consistency() {
    ...
    if scheduler_error.contains(non_existent_id) {
        ...
        println!(
            "🟡 WARNING: Scheduler error doesn't mention checkpoint ID: {}",
            scheduler_error
        );
    }
    ...
    if snapshots_error.contains(non_existent_id) {
        ...
        println!(
            "🟡 WARNING: Snapshots error doesn't mention checkpoint ID: {}",
            snapshots_error
        );
    }
}
...
async fn attack_concurrent_restoration() {
    ...
    if success_count == 0 {
        ...
        println!(
            "🔴 CRITICAL: Inconsistent concurrent restoration: {} succeeded, {} failed",
            success_count,
            10 - success_count
        );
