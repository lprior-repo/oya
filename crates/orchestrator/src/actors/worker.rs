...
mod tests {
    ...
    fn test_opencode_config_accepts_workspace_path() {
        // Verify OpencodeConfig can accept workspace_path via builder
        let workspace_path = PathBuf::from("/tmp/oya-workspaces/test-bead");
        let config = oya_opencode::OpencodeConfig::default().working_dir(workspace_path.clone());

        assert_eq!(
            config.working_directory,
            Some(workspace_path),
            "OpencodeConfig working_directory should be set to workspace_path"
        );
    }

    #[test]
    fn test_opencode_client_created_with_workspace_config() {
        // Verify OpencodeClient can be created with workspace config
        let workspace_path = PathBuf::from("/tmp/oya-workspaces/test-bead-client");
        let config = oya_opencode::OpencodeConfig::default().working_dir(workspace_path);

        let client_result = oya_opencode::OpencodeClient::with_config(config);
        assert!(
            client_result.is_ok(),
            "OpencodeClient should be created with workspace config"
        );
    }

    #[test]
    fn test_bead_execution_to_opencode_integration() {
        // Integration test: verify BeadExecutionContext -> OpencodeConfig flow
        let bead_id = "test-bead-integration";
        let workspace_path = PathBuf::from("/tmp/oya-workspaces").join(bead_id);

        // Create execution context
        let exec_ctx = BeadExecutionContext::new(bead_id, workspace_path.clone());

        // Verify context has workspace_path
        assert_eq!(exec_ctx.bead_id(), bead_id);
        assert_eq!(exec_ctx.workspace_path(), &workspace_path);

        // Create OpencodeConfig with workspace_path from context
        let opencode_config =
            oya_opencode::OpencodeConfig::default().working_dir(exec_ctx.workspace_path().clone());

        // Verify config has correct working_directory
        assert_eq!(
            opencode_config.working_directory,
            Some(workspace_path),
            "OpencodeConfig should have workspace_path from BeadExecutionContext"
        );
    }
}
