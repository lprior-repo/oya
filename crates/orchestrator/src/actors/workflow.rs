...
impl Clone for WorkflowMessage {
    fn clone(&self) -> Self {
        match self {
            Self::AddBead { bead_id } => Self::AddBead { bead_id: bead_id.clone() },
            Self::MarkCompleted { bead_id } => Self::MarkCompleted { bead_id: bead_id.clone() },
            Self::Rehydrate { events } => Self::Rehydrate { events: events.clone() },
...
mod core {
    ...
    pub fn handle(state: WorkflowState, msg: WorkflowMessage) -> (WorkflowState, Vector<WorkflowEffect>) {
        ...
        match msg {
            ...
            WorkflowMessage::GetReadyBeads { reply } => {
                ...
                effects.push_back(WorkflowEffect::ReplyReadyBeads { reply, beads: ready });
            }
