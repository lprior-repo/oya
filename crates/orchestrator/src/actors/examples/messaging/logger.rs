...
mod tests {
    ...
    async fn send_pattern_log_message() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (actor, handle): (ActorRef<LoggerMessage>, _) = spawn_logger().await?;
        ...
        let entries: Vector<LogEntry, ArcK> = rx.await?;
    ...
    async fn send_pattern_log_with_level() -> Result<(), Box<dyn std::error::Error>> {
        // Given: A logger actor
        let (actor, handle): (LoggerActorHandle, _) = spawn_logger().await?;
        ...
        let levels: Vec<LogLevel> = entries.iter().map(|e: &LogEntry| e.level()).collect();
        assert_eq!(
            levels,
    ...
    async fn send_pattern_clear_logs() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result: Vector<LogEntry, ArcK> = rx.await?;
