...
use crate::persistence::{OrchestratorStore, StoreConfig, CheckpointRecord};
use crate::replay::checkpoint::{CheckpointManager, CheckpointConfig};
...
impl CheckpointAttackHarness {
    ...
    pub async fn create_malformed_checkpoint(&mut self, data: &str) -> Result<CheckpointRecord, Box<dyn std::error::Error + Send + Sync>> {
        self.manager.create_checkpoint(data, None).await.map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)
    }
}
...
async fn attack_happy_path_basic_restoration() {
    ...
    let checkpoint = manager.create_checkpoint(valid_data, None).await.expect("Failed to create checkpoint");
    ...
    assert_eq!(restored_value.get("active_workflows").unwrap().as_array().unwrap().len(), 1);
    assert_eq!(restored_value.get("sequence").unwrap().as_u64().unwrap(), 42);
}
...
async fn attack_happy_path_with_snapshots() {
    ...
    let _checkpoint = manager.create_checkpoint(scheduler_data, Some(snapshots_data)).await
    ...
    let snapshots: Result<Option<serde_json::Value>, _> = manager.restore_workflow_snapshots().await;
    ...
    assert!(snapshots.unwrap().get("wf-1").is_some(), "wf-1 should be in snapshots");
}
...
async fn attack_empty_checkpoint_id() {
    ...
    assert!(error_msg.contains("not found") || error_msg.contains("checkpoint"), 
            "Error should indicate checkpoint not found, got: {}", error_msg);
}
...
async fn attack_malformed_checkpoint_id() {
    ...
    for id in malformed_ids {
        ...
        assert!(result.is_err(), "Malformed checkpoint ID '{}' should fail", id);
    }
}
...
async fn attack_uninitialized_database() {
    ...
    assert!(result.is_err(), "Restoration should fail without initialized schema");
...
async fn attack_corrupted_json_in_checkpoint() {
    ...
    assert!(result.is_err(), "Restoration should fail with corrupted JSON");
    
    // Verify error is a serialization error
    let error = result.unwrap_err();
    let error_msg = error.to_string();
    assert!(error_msg.contains("serialization") || error_msg.contains("deserialize") || error_msg.contains("json"),
            "Error should indicate JSON deserialization failed, got: {}", error_msg);
}
...
async fn attack_concurrent_restoration() {
    ...
    assert!(success_count == 0 || success_count == 10, 
...
async fn attack_type_safety_violations() {
    ...
    assert!(as_string.is_err(), "Restoring JSON object as String should fail");
    
    let as_number: Result<u64, _> = manager.restore_scheduler_state().await;
    assert!(as_number.is_err(), "Restoring JSON object as u64 should fail");
    
    // Should only work as compatible types
    let as_json: Result<serde_json::Value, _> = manager.restore_scheduler_state().await;
    assert!(as_json.is_ok(), "Restoring as serde_json::Value should work");
}
...
async fn attack_error_message_consistency() {
    ...
    assert!(scheduler_result.is_err(), "Scheduler restoration should fail");
    assert!(snapshots_result.is_err(), "Snapshot restoration should fail");
    ...
    assert!(scheduler_error.contains(non_existent_id), 
            "Scheduler error should mention checkpoint ID: {}", scheduler_error);
    assert!(snapshots_error.contains(non_existent_id), 
            "Snapshots error should mention checkpoint ID: {}", snapshots_error);
    ...
    match (scheduler_result, snapshots_result) {
        (Err(scheduler_err), Err(snapshots_err)) => {
            ...
            assert!(std::mem::discriminant(&scheduler_err) == std::mem::discriminant(&snapshots_err),
...
async fn cross_command_consistency_latest_vs_by_id() {
    ...
    let cp1 = manager.create_checkpoint(data1, None).await.expect("Failed to create cp1");
    let cp2 = manager.create_checkpoint(data2, None).await.expect("Failed to create cp2");
    
    // Restore latest and by ID - should be consistent
    let latest: Result<serde_json::Value, _> = manager.restore_scheduler_state().await;
    let by_id: Result<serde_json::Value, _> = manager.restore_scheduler_state_by_id(&cp2.checkpoint_id).await;
    ...
    assert_eq!(latest_value, by_id_value, "Latest and by-ID restoration should be consistent");
}
...
async fn attack_workflow_snapshots_none_consistency() {
    ...
    let _checkpoint = manager.create_checkpoint(data, None).await.expect("Failed to create checkpoint");
    ...
    let by_id: Result<Option<String>, _> = manager.restore_workflow_snapshots_by_id(&_checkpoint.checkpoint_id).await;
    
    assert!(latest.is_ok(), "Latest snapshots restoration should succeed");
    ...
    assert_eq!(latest_value, by_id_value, "None values should be consistent");
}
...
mod attack_reports {
    ...
    async fn generate_attack_report() {
        ...
        match CheckpointAttackHarness::new_uninitialized().await {
            Some(harness) => {
                let result: Result<serde_json::Value, _> = harness.manager.restore_scheduler_state().await;
                match result {
                    Ok(_) => println!("🔴 CRITICAL: Restoration succeeded without schema initialization!"),
        ...
        match CheckpointAttackHarness::new().await {
            Some(harness) => {
                let result: Result<serde_json::Value, _> = harness.manager.restore_scheduler_state_by_id("").await;
                match result {
