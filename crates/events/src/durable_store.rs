...
/// Batch append error with detailed context for debugging.
#[derive(Debug, Error, Clone, PartialEq, Eq)]
pub enum AppendBatchError {
    ...
    #[error("failed to serialize event at index {index} (id: {event_id}): {reason}")]
    SerializationFailed {
        index: usize,
        event_id: String,
        reason: String,
    ...
    #[error("failed to write batch to wal: {0}")]
    WalWriteFailed(String),
    ...
    #[error("database batch write failed: {0}")]
    DatabaseWriteFailed(String),
...
impl From<AppendError> for AppendBatchError {
    fn from(err: AppendError) -> Self {
        match err {
            AppendError::WalOpenFailed(msg) => Self::WalOpenFailed(msg),
            AppendError::WalWriteFailed(msg) => Self::WalWriteFailed(msg),
            AppendError::WalSyncFailed(msg) => Self::WalSyncFailed(msg),
            AppendError::WalCloseFailed(msg) => Self::WalCloseFailed(msg),
            AppendError::SerializationFailed(msg) => Self::SerializationFailed {
                index: 0,
                event_id: "unknown".to_string(),
                reason: msg,
            },
            AppendError::DatabaseWriteFailed(msg) => Self::DatabaseWriteFailed(msg),
        }
    }
}
...
impl DurableEventStore {
    ...
    /// Append multiple events in a single atomic operation with fsync amortization.
    ///
    /// # Preconditions
    /// - Batch is non-empty
    /// - Batch size <= 1000
    ...
    pub async fn append_batch(
        ...
        let serialized_events: std::result::Result<Vec<_>, _> = events
            .iter()
            .enumerate()
            .map(|(index, event)| {
                SerializedEvent::from_bead_event(event).map_err(|e| {
        ...
        let serialized_events = serialized_events?;

        // Collect event IDs in order
        let event_ids: Vec<crate::types::EventId> =
            events.iter().map(BeadEvent::event_id).collect();

        // Prepare pairs for WAL write
        let event_pairs: Vec<_> = events.iter().zip(serialized_events.iter()).collect();

        // Write batch to WAL with single fsync
        self.append_batch_to_wal(&event_pairs).await?;

        // Insert batch into database
        self.insert_batch_to_db(&serialized_events).await?;
    ...
    ) -> std::result::Result<(), AppendBatchError> {
        let _guard = self.wal_lock.lock().await;
        ...
        match events.first() {
            Some((first_event, _)) => {
                ...
                for (_event, serialized) in events {
                    let serialized_data = bincode::serialize(&serialized).map_err(|e| {
                        AppendBatchError::SerializationFailed {
                            index: 0,
            ...
            None => Err(AppendBatchError::EmptyBatch),
        }
    }
