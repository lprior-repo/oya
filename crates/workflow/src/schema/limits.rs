...
+    /// # Errors
+    ///
+    /// Returns an error if capacity is non-positive or refill rate is non-positive.
+    pub const fn new(capacity: i64, refill_rate: f64) -> Result<Self> {
         if capacity <= 0 {
...
+    /// # Errors
+    ///
+    /// Returns an error if the resource identifier is empty.
+    pub fn create(resource_id: String, config: &TokenBucketConfig) -> Result<Self> {
...
+    /// # Errors
+    ///
+    /// Returns an error if max concurrent is non-positive.
+    pub const fn new(max_concurrent: i64) -> Result<Self> {
         if max_concurrent <= 0 {
...
+        "UPDATE token_bucket:{table_id} SET \
...
+        "UPDATE concurrency_limit:{table_id} SET \
