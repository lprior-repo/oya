...
impl Viewport {
    ...
    pub fn new(width: f32, height: f32) -> Result<Self, String> {
        if !width.is_finite() || width <= 0.0 {
            return Err(format!("Canvas width must be positive and finite, got: {}", width));
        }
        if !height.is_finite() || height <= 0.0 {
            return Err(format!("Canvas height must be positive and finite, got: {}", height));
        }
...
mod tests {
    ...
    fn test_large_pan_offsets() -> Result<(), String> {
        let viewport = Viewport::with_transform(1200.0, 800.0, 10000.0, -10000.0, ZoomLevel::default())?;
    ...
    fn test_negative_pan_offsets() -> Result<(), String> {
        let viewport = Viewport::with_transform(1200.0, 800.0, -200.0, -100.0, ZoomLevel::default())?;
