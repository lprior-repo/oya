...
mod tests {
    ...
    async fn test_web_client_health_check_network_error() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        // Accept various network-related errors - the exact error type depends on
        // the HTTP client implementation and network conditions
        match result {
            ...
            Err(WebClientError::Network { .. }) => {
                // Generic network error - also acceptable
            }
            Err(WebClientError::Tls { .. }) => {
                // TLS errors are also network-related
            }
            Err(other) => {
                // Unexpected error type - convert to Result error
                return Err(format!("Unexpected error type from health_check: {:?}", other).into());
            }
            Ok(_) => {
                // Should not happen - the assertion above would catch this
                return Err("health_check should return error for invalid endpoint".into());
            }
        }
