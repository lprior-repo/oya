...
mod tests {
    ...
    #[test]
    fn test_ipc_client_connect_accepts_hostname_with_port() {
        let result = IpcClient::connect("localhost:5555");
        assert!(result.is_ok());
    }

    #[test]
    fn test_ipc_client_connect_rejects_control_chars() {
        let result = IpcClient::connect("127.0.0.1:5555\n");
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_ipc_client_connect_rejects_stdio_with_control_chars() {
        let result = IpcClient::connect("stdio://zellij\n");
        assert!(result.is_err());
    }
}
