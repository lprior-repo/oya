...
- Keep gate execution moon-only in `src/main.rs`: `moon run :check|:test|:quick|:ci`; ship gate order is `:ci -> (optional zjj dry-run) -> :quick -> :test`.
- Keep workspace provisioning order for workspace stages in `src/main.rs`: `zjj queue --add <ws> --bead <id>` before `zjj add <ws> --idempotent`; workspace names come from `build_zjj_workspace_name` and remain <= 64 chars.
