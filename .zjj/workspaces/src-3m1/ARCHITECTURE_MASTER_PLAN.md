...
7. Codanna MCP is the code discovery engine (symbols, callers, calls, impact).
8. The planner companion app is the hard front door to the pipeline.
...
## Functional Rust Integration

All AI-generated code must follow the functional Rust guidelines (see `docs/FUNCTIONAL_RUST.md`).
...
- Returns Effects: Pure functions return `im::Vector<Effect>` describing what *should* happen.
...
- Executes Effects: Interprets the `im::Vector<Effect>` and performs I/O.
