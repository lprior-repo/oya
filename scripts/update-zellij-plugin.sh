...
WASM_SOURCE="${PROJECT_ROOT}/target/wasm32-wasip1/release/zellij_frontend.wasm"
WASM_DEST="${HOME}/.local/share/zellij/plugins/zellij_frontend.wasm"

echo "🔄 Rebuilding Zellij plugin..."
cd "$PROJECT_ROOT"
cargo build --release --target wasm32-wasip1 -p zellij-frontend 2>&1 | grep -E "(Compiling|Finished|error)" || true
