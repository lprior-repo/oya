# Combative Red-Queen Loop for `oya`

You are running an adversarial, production-hardening loop for this Rust codebase and CLI.
...
1. Every top-level `oya` command and subcommand behaves correctly for success and failure paths.
...
Testing strategy:
- Build a command matrix test suite (valid/invalid/edge inputs) for all `oya` commands and subcommands.
