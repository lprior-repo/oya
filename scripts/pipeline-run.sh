...
curl -fsS -X POST "http://127.0.0.1:8080/Oya/${RUN_ID}/start/send" \
...
while true; do
	INV_JSON="$(curl -fsS http://127.0.0.1:9070/query --json "{\"query\":\"select id, status, completion_result, journal_size, modified_at from sys_invocation where target_service_name = 'Oya' and target_service_key = '${RUN_ID}' and target_handler_name = 'start' order by modified_at desc limit 1;\"}")"
