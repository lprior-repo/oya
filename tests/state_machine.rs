...
/// Test all stage transitions in the happy path (five canonical stages)
#[tokio::test]
async fn test_all_stage_transitions() {
    // Five canonical stages: Contract -> AcceptanceTest -> Implementation -> Review -> ShipGate
    let test_cases = vec![
        (StageName::Contract, Some(StageName::AcceptanceTest)),
        (StageName::AcceptanceTest, Some(StageName::Implementation)),
        (StageName::Implementation, Some(StageName::Review)),
        (StageName::Review, Some(StageName::ShipGate)),
...
async fn test_merge_conflict_fails_after_max_attempts() {
    ...
    for attempt in 1..=2 {
        let result = orch
            ...
            .unwrap();
        if attempt < 2 {
            assert_eq!(result.next_stage, Some(StageName::Review));
        }
    }
}

/// Test complete pipeline execution (happy path simulation with five stages)
#[tokio::test]
async fn test_complete_pipeline_simulation() {
    ...
    // Verify all stages were called (five canonical stages)
