...
files = [
    ...
    "crates/zellij-frontend/src/web_client.rs",
]
...
for f in files:
    try:
        ...
        # Remove lines with allow directives
        content = re.sub(r"#\!\[allow\(clippy::unwrap_used\)\]\n?", "", content)
        content = re.sub(r"#\!\[allow\(clippy::expect_used\)\]\n?", "", content)
        content = re.sub(
            r"#\[allow\(clippy::expect_used, clippy::unwrap_used\)\]\n?", "", content
        )
        content = re.sub(
            r"#\[allow\(clippy::unwrap_used, clippy::expect_used\)\]\n?", "", content
        )
        content = re.sub(r"#\!\[allow\(clippy::panic\)\]\n?", "", content)
        content = re.sub(r"#\!\[allow\(clippy::indexing_slicing\)\]\n?", "", content)
        content = re.sub(r"#\[allow\(clippy::panic\)\]\n?", "", content)
