...
test_list_one_session() {
    ...
    run_cmd "zjj add --no-zellij test-session-1"
    local output
    output=$(run_cmd "zjj list")
...
test_rename_to_existing() {
    ...
    run_cmd "zjj add --no-zellij test-session-2"
...
test_rename_special_chars() {
    log "\n=== TEST 7: Rename with special characters ==="
    run_cmd "zjj add --no-zellij test-session-3"
...
test_remove_active() {
    log "\n=== TEST 8: Remove active session ==="
    run_cmd "zjj add --no-zellij to-be-removed"
...
test_massive_list() {
    ...
    for i in {1..100}; do
        zjj add --no-zellij "bulk-session-$i" >/dev/null 2>&1 || warn "Failed to create bulk-session-$i"
    done
...
test_rapid_operations() {
    ...
    for i in {1..20}; do
        zjj add --no-zellij "rapid-$i" >/dev/null 2>&1 || true
...
test_unicode_names() {
    ...
    run_cmd "zjj add --no-zellij 'session-café'" || warn "Failed to create café session"
    run_cmd "zjj add --no-zellij 'session-test-123'" || warn "Failed to create test-123 session"
    run_cmd "zjj add --no-zellij 'session_test'" || warn "Failed to create session_test"
...
test_concurrent_simulation() {
    ...
    for i in {1..10}; do
        zjj add --no-zellij "concurrent-a-$i" >/dev/null 2>&1 &
    done
    wait
    ...
    for i in {1..10}; do
        ...
        zjj add --no-zellij "concurrent-c-$i" >/dev/null 2>&1 &
    done
    wait
...
test_rename_same_name() {
    ...
    run_cmd "zjj add --no-zellij test-rename-same"
