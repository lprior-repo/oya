...
pub use durable_store::{AppendBatchError, AppendError, connect, ConnectionConfig, DurableEventStore};
...
// Re-export replay types (already re-exported from replay module)
pub use replay::{
    apply_event, apply_events, ApplyContext, EventSourcedState,
    create_tracker, is_transient_error, EventFilter, EventLoader, LoadError, RecoveryConfig,
    RecoveryStrategy, ReplayProgress, ReplayState, ReplayTracker, RetryPolicy,
};

// Re-export db types
pub use db::{DbError, SurrealDbClient, SurrealDbConfig};

// Re-export scheduler types if any (from orchestrator if needed)
...
pub fn init_telemetry_json() -> Result<()> {
    use oya_telemetry::config::TelemetryConfig;
