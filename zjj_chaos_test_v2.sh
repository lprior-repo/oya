#!/bin/bash
################################################################################
# ZJJ CHAOS TEST SUITE V2 - QA Agent #10
...
# Backup directory
BACKUP_DIR="$HOME/.zjj_backup_$$"

# Save current .zjj if it exists
save_current_state() {
    if [ -d ".zjj" ] && [ "$(pwd)" != "$HOME" ]; then
        mv .zjj "$BACKUP_DIR" 2>/dev/null || true
    fi
}
...
restore_current_state() {
    if [ -d "$BACKUP_DIR" ]; then
        mv "$BACKUP_DIR" .zjj 2>/dev/null || true
    fi
}
...
setup_test_repo() {
    ...
    # Initialize git repo (simpler than JJ for testing)
    git init -q
    ...
    # Initialize zjj
    zjj init --colocate -q 2>&1 || zjj init -q 2>&1 || true
...
test_invalid_arguments() {
    ...
    local result
    result=$(printf "test\x00name" 2>/dev/null | timeout 5s zjj add "$(printf "test\x00name")" 2>&1 || echo "timeout/error")
    if echo "$result" | grep -qiE "error|invalid|timeout"; then
        log_pass "Null bytes rejected or handled"
    else
        log_vulnerability "Null bytes not sanitized - potential injection vector"
        log_fail "Null bytes" "Should reject null bytes"
    fi

    # Test 3: Massive input (1MB string)
    log_test "Massive input (1MB session name)"
    repo=$(setup_test_repo "massive_input")
    local massive=$(python3 -c "print('A' * 1048576)")
    local result
    result=$(timeout 10s zjj add "$massive" 2>&1 || echo "timeout")
    if echo "$result" | grep -qiE "error|too long|invalid|timeout"; then
        log_pass "Massive input rejected"
    else
        if echo "$result" | grep -qiE "created|added"; then
            log_vulnerability "Accepted 1MB input - DoS via memory exhaustion"
    ...
    local result
    result=$(timeout 5s zjj add "$combined" 2>&1 || echo "timeout")
    if echo "$result" | grep -qiE "error|invalid|timeout"; then
...
test_command_injection() {
    ...
    if echo "$output" | grep -qiE "error|invalid"; then
        ...
        # Check if ls was actually executed by looking for directory listing output
...
test_concurrent_operations() {
    ...
    cd "$repo"
    local count=$(zjj list 2>/dev/null | grep -cE "concurrent_" || echo 0)
    if [ $count -ge 40 ]; then
    ...
    # Test 18: 100 rapid status queries
    ...
    # Test 19: Concurrent add/remove of same session
...
test_state_corruption() {
    ...
    # Test 20: Delete .zjj directory during operation
    ...
    local db_file=$(find .zjj -type f \( -name "*.db" -o -name "*.sqlite" -o -name "*.json" \) 2>/dev/null | head -1)
    if [ -n "$db_file" ]; then
...
test_edge_cases() {
    ...
    # Test 23: Single character session names
...
################################################################################
# TEST CATEGORY 7: UNICODE ATTACKS
################################################################################

test_unicode_attacks() {
    echo -e "\n${CYAN}=== CATEGORY 7: UNICODE ATTACKS ===${NC}"

    # Test 27: Emoji in session names
    log_test "Emoji in session names"
    local repo=$(setup_test_repo "emoji")
    cd "$repo"
    ...
    # Test 28: Very long Unicode names
    log_test "Long Unicode name (100 chars)"
    repo=$(setup_test_repo "long_unicode")
    cd "$repo"

    local long_name=$(python3 -c "print('测试' * 50)")
    local result
    result=$(timeout 5s zjj add "$long_name" 2>&1 || echo "timeout")
    if echo "$result" | grep -qiE "error|too long|invalid|timeout"; then
        log_pass "Long Unicode name rejected"
    else
        log_pass "Long Unicode name accepted"
    fi

    # Test 29: Mixed scripts
    log_test "Mixed scripts (Latin + Cyrillic + Arabic)"
    repo=$(setup_test_repo "mixed_scripts")
    cd "$repo"

    if zjj add "testаестمبح" 2>&1 | grep -qiE "error|invalid"; then
        log_pass "Mixed scripts rejected"
    else
        log_vulnerability "Mixed scripts not filtered - homoglyph risk"
        log_fail "Mixed scripts" "Should normalize or warn"
    fi
}
...
test_resource_exhaustion() {
    echo -e "\n${CYAN}=== CATEGORY 8: RESOURCE EXHAUSTION ===${NC}"

    # Test 30: Create many sessions
    log_test "Create 200 sessions"
    local repo=$(setup_test_repo "many_sessions")
    cd "$repo"
    ...
    # Test 31: Rapid add/remove cycles
...
main() {
    echo -e "${MAGENTA}"
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║   ZJJ CHAOS TEST SUITE V2 - QA Agent #10                  ║"
