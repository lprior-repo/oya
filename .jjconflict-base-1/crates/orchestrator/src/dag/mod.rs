...
impl WorkflowDAG {
        ...
        sccs.into_iter()
            .filter(|scc| {
                ...
                if scc.len() > 1 {
                    ...
                    let bead_id = &scc[0];
                    if let Some(node_idx) = self.node_map.get(bead_id) {
                        self.graph
    ...
    /// Find all strongly connected components using Tarjan's SCC algorithm.
    ///
    /// This is the core implementation of Tarjan's algorithm, which finds all
    /// strongly connected components in linear time O(V + E).
    ///
    /// # Algorithm
    ///
    /// Tarjan's algorithm performs a single DFS pass, maintaining:
    /// - `index`: DFS discovery order for each node
    /// - `low_link`: smallest index reachable from the node (including through back edges)
    /// - `stack`: nodes in the current SCC being explored
    /// - `on_stack`: track which nodes are on the stack
    ///
    /// When a node's low_link equals its index, we've found an SCC root.
    ///
    /// # Returns
    ///
    ...
    /// ```
    #[must_use]
    pub fn tarjan_scc(&self) -> Vec<Vec<BeadId>> {
        struct TarjanState {
            index: usize,
            stack: Vec<BeadId>,
            on_stack: HashSet<BeadId>,
            indices: HashMap<BeadId, usize>,
            low_links: HashMap<BeadId, usize>,
            sccs: Vec<Vec<BeadId>>,
        }
        ...
        impl TarjanState {
            ...
            fn push_stack(&mut self, bead_id: BeadId) {
                self.on_stack.insert(bead_id.clone());
                self.stack.push(bead_id);
            }

            fn pop_stack_until(&mut self, bead_id: &BeadId) -> Vec<BeadId> {
                let mut scc = Vec::new();
                loop {
        ...
        // Visit each node that hasn't been visited
        for node_idx in self.graph.node_indices() {
            let bead_id = match self.graph.node_weight(node_idx) {
                Some(id) => id.clone(),
                None => continue,
            };

            if state.indices.contains_key(&bead_id) {
                continue;
            }

            // Recursive DFS using a manual stack to avoid recursion depth issues
            let mut work_stack = vec![(bead_id.clone(), 0)];

            while let Some((current_id, phase)) = work_stack.pop() {
                match phase {
                    0 => {
                        // First visit: assign index and low_link
                        if state.indices.contains_key(&current_id) {
                            continue;
                        }
                        ...
                        state.indices.insert(current_id.clone(), current_index);
                        state.low_links.insert(current_id.clone(), current_index);
                        state.push_stack(current_id.clone());

                        // Get neighbors
                        let node_idx = match self.node_map.get(&current_id) {
                            Some(idx) => *idx,
                            None => continue,
                        };

                        // Push post-processing phase
                        work_stack.push((current_id.clone(), 1));

                        // Push neighbors in reverse order to maintain original order
                        let neighbors: Vec<BeadId> = self
                            .graph
                            .neighbors_directed(node_idx, Direction::Outgoing)
                            .filter_map(|idx| self.graph.node_weight(idx).cloned())
                            .collect();
                    ...
                    1 => {
                        // Post-processing: check if this is an SCC root
                        let current_index = match state.indices.get(&current_id) {
                            Some(idx) => *idx,
                            None => continue,
                        };

                        let low_link = match state.low_links.get(&current_id) {
                            Some(link) => *link,
                            None => continue,
                        };

                        if low_link == current_index {
                            let scc = state.pop_stack_until(&current_id);
                            state.sccs.push(scc);
                        }
                    }
                    _ => unreachable!(),
    ...
    /// Validate all dependency relationships in the DAG.
    ///
    /// This method performs comprehensive validation of the graph structure to ensure
    /// all dependency relationships are valid and the graph is a proper DAG.
    ///
    /// # Validation Checks
    ///
    /// - No cycles exist in the graph (including indirect cycles)
    /// - No self-loops (a node depending on itself)
    /// - All edges point to valid nodes
    /// - The graph structure is internally consistent
    ///
    /// # Returns
    ///
    /// * `Ok(())` - All dependency relationships are valid
    /// * `Err(DagError::CycleDetected)` - A cycle was found in the graph
    /// * `Err(DagError::SelfLoopDetected)` - A self-loop was detected
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// dag.add_node("b".to_string())?;
    /// dag.add_node("c".to_string())?;
    /// dag.add_dependency("a".to_string(), "b".to_string(), DependencyType::BlockingDependency)?;
    /// dag.add_dependency("b".to_string(), "c".to_string(), DependencyType::BlockingDependency)?;
    ///
    /// assert!(dag.validate_dependency_relationships().is_ok());
    ///
    /// // Creating a cycle will fail validation
    /// dag.add_dependency("c".to_string(), "a".to_string(), DependencyType::BlockingDependency)?;
    /// assert!(dag.validate_dependency_relationships().is_err());
    /// # Ok(())
    /// # }
    /// ```
    pub fn validate_dependency_relationships(&self) -> DagResult<()> {
        // Check for self-loops
        self.validate_no_self_loops()?;

        // Check for cycles using Tarjan's SCC algorithm
        let cycles = self.find_cycles();
        if !cycles.is_empty() {
            // Find the first cycle and report it
            let cycle_nodes = cycles.into_iter().next().unwrap_or_else(|| Vec::new());
            return Err(DagError::cycle_detected(cycle_nodes));
        }
...
mod tests {
    ...
    // ==================== Validate Dependency Relationships Tests ====================

    #[test]
    fn test_validate_dependency_relationships_empty_dag() {
        let dag = WorkflowDAG::new();
        let result = dag.validate_dependency_relationships();
        assert!(result.is_ok(), "Empty DAG should be valid");
    }

    #[test]
    fn test_validate_dependency_relationships_valid_dag() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
    ...
    #[test]
    fn test_validate_dependency_relationships_detects_cycles() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
        ...
        let result = dag.validate_dependency_relationships();
        assert!(result.is_err(), "DAG with cycle should fail validation");
        assert!(matches!(result, Err(DagError::CycleDetected(_))));
        Ok(())
    }

    #[test]
    fn test_validate_dependency_relationships_diamond_pattern() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
    ...
    #[test]
    fn test_validate_dependency_relationships_multiple_roots() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
        ...
        let result = dag.validate_dependency_relationships();
        assert!(result.is_ok(), "Multiple roots should be valid");
        Ok(())
    }

    #[test]
    fn test_validate_dependency_relationships_with_preferred_order() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
        ...
        assert!(
            result.is_ok(),
            "PreferredOrder dependencies should be valid"
        );
        Ok(())
    }

    #[test]
    fn test_validate_dependency_relationships_disconnected_components() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
        ...
        let result = dag.validate_dependency_relationships();
        // Disconnected components are still valid DAGs
        assert!(result.is_ok(), "Disconnected components should be valid");
        Ok(())
    }
    ...
    #[test]
    fn test_tarjan_scc_single_node() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
    ...
    #[test]
    fn test_tarjan_scc_dag_no_cycles() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
        ...
        let all_nodes: Vec<_> = sccs.iter().flatten().collect();
        assert!(all_nodes.contains(&&"a".to_string()));
        assert!(all_nodes.contains(&&"b".to_string()));
        assert!(all_nodes.contains(&&"c".to_string()));
        Ok(())
    }

    #[test]
    fn test_tarjan_scc_simple_cycle() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_edge(
    ...
    #[test]
    fn test_tarjan_scc_three_node_cycle() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
    ...
    #[test]
    fn test_tarjan_scc_multiple_sccs() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        // First SCC: a <-> b
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        ...
        assert!(all_nodes.contains(&&"b".to_string()));
        assert!(all_nodes.contains(&&"c".to_string()));
        assert!(all_nodes.contains(&&"d".to_string()));
        Ok(())
    }

    #[test]
    fn test_tarjan_scc_dag_with_bridge() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        // SCC1: a <-> b
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        ...
        // Bridge node: c (connects the two SCCs)
        dag.add_node("c".to_string())?;
        dag.add_edge(
            "b".to_string(),
            "c".to_string(),
        ...
        let sccs = dag.tarjan_scc();
        // Should find 3 SCCs: {a,b}, {c}, {d,e}
        assert_eq!(sccs.len(), 3);

        // Check sizes
        let sizes: Vec<_> = sccs.iter().map(|scc| scc.len()).collect();
        assert!(sizes.contains(&2)); // Two SCCs with 2 nodes
        assert!(sizes.contains(&1)); // One SCC with 1 node
        ...
        assert!(all_nodes.contains(&&"c".to_string()));
        assert!(all_nodes.contains(&&"d".to_string()));
        assert!(all_nodes.contains(&&"e".to_string()));
        Ok(())
    }

    #[test]
    fn test_tarjan_scc_complex_diamond() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
    ...
    #[test]
    fn test_tarjan_scc_self_loop() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;

        // Create a self-loop by adding the edge directly to the graph
        // Note: add_edge prevents self-loops, so we need to test via the graph
        let node_idx = dag.node_map.get(&"a".to_string()).copied();
        if let Some(idx) = node_idx {
            dag.graph
                .add_edge(idx, idx, DependencyType::BlockingDependency);
        }
        ...
        assert_eq!(sccs.len(), 1);
        assert_eq!(sccs[0].len(), 1);
        assert_eq!(sccs[0][0], "a".to_string());
        Ok(())
    }
    ...
    #[test]
    fn test_get_ready_nodes_basic() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
    ...
    fn test_query_performance_100_nodes() -> DagResult<()> {
        ...
        // Test get_ready_nodes performance
        let completed = HashSet::new();
        let start = Instant::now();
        let ready = dag.get_ready_nodes(&completed);
        let duration = start.elapsed();
    ...
    #[test]
    fn test_get_ready_nodes_with_completed_set() -> DagResult<()> {
        let mut dag = WorkflowDAG::new();
        dag.add_node("a".to_string())?;
        dag.add_node("b".to_string())?;
        dag.add_node("c".to_string())?;
        ...
        // Create chain: a -> b -> c, d is separate root
        dag.add_edge(
            "a".to_string(),
            "b".to_string(),
            DependencyType::BlockingDependency,
        ...
        // After a and d complete, b is ready
        let mut completed = HashSet::new();
        completed.insert("a".to_string());
