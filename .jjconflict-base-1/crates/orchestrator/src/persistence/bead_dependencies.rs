...
impl OrchestratorStore {
    ...
    pub async fn save_dependency_edge(
        ...
        let table_name = match edge.relation_type {
        ...
        let result: Option<DependencyEdge> = self
            .db()
            .query("INSERT INTO $table CONTENT $input RETURN AFTER")
    ...
    pub async fn delete_dependency_edge(
        ...
        let result: Option<DependencyEdge> = self
            .db()
            .query(
                "DELETE FROM $table WHERE bead_id = $bead_id AND target_bead_id = $target_bead_id RETURN BEFORE"
            )
            .bind(("table", table_name))
            .bind(("bead_id", bead_id_owned))
