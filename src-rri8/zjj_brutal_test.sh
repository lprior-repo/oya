...
test_list_zero_sessions() {
    log "\n=== TEST 1: List with 0 sessions ==="
    run_cmd "zjj list"
    run_cmd "zjj list --json" || run_cmd "zjj list -j" || true
}
...
test_list_one_session() {
    ...
    run_cmd "zjj list"
    run_cmd "zjj list | grep -q 'test-session-1'" "false"
}
...
test_status_single() {
    ...
    run_cmd "zjj status --json test-session-1" || run_cmd "zjj status -j test-session-1" || true
}
...
test_rename_basic() {
    ...
    run_cmd "zjj list | grep -q 'renamed-session-1'" "false"
    run_cmd "zjj list | grep -q 'test-session-1'" "true"
}
...
test_remove_active() {
    ...
    run_cmd "zjj list | grep -q 'to-be-removed'" "true"
}
...
test_massive_list() {
    ...
    for i in {1..100}; do
        zjj add "bulk-session-$i" >/dev/null 2>&1 || true
    done
    ...
    run_cmd "zjj list | wc -l"
    run_cmd "zjj list | grep -c 'bulk-session'" "false"
}
...
test_status_during_ops() {
    ...
    run_cmd "zjj status --all" || run_cmd "zjj status -a" || true
}
...
test_unicode_names() {
    ...
    run_cmd "zjj add 'session-α'" || true
    run_cmd "zjj add 'session-日本語'" || true
    run_cmd "zjj add 'session-😀'" || true
    run_cmd "zjj add 'session-café'" || true
    run_cmd "zjj list | grep -q 'session-'" "false"
}

# Test 15: Empty names and edge cases
test_edge_cases() {
    log "\n=== TEST 15: Edge cases ==="
    run_cmd "zjj rename '' newname" "true" || true
    run_cmd "zjj rename bulk-session-1 ''" "true" || true
    run_cmd "zjj remove -y ''" "true" || true
}
...
test_list_formatting() {
    ...
    run_cmd "zjj list"
    run_cmd "zjj list --json" || run_cmd "zjj list -j" || true
    run_cmd "zjj list --verbose" || run_cmd "zjj list -v" || true
}

# Test 18: Status with filters
test_status_filters() {
    log "\n=== TEST 18: Status with filters ==="
    run_cmd "zjj status --active" || run_cmd "zjj status -A" || true
    run_cmd "zjj status --inactive" || run_cmd "zjj status -I" || true
}
...
test_cleanup_all() {
    ...
    zjj list 2>/dev/null | grep -oP '\[\K[^\]]+' | while read -r session; do
        zjj remove -y "$session" 2>/dev/null || true
    done

    run_cmd "zjj list"
    run_cmd "zjj status --all" || true
}
