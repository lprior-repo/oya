...
impl WorkflowDAG {
    ...
    pub fn tarjan_scc(&self) -> Vec<Vec<BeadId>> {
        ...
        impl<'a> TarjanState<'a> {
            ...
            fn strongconnect(&mut self, bead_id: BeadId) {
                // Set the depth index for v to the smallest unused index
                self.indices.insert(bead_id.clone(), self.index);
                self.low_links.insert(bead_id.clone(), self.index);
                self.index += 1;
                self.stack.push(bead_id.clone());
                self.on_stack.insert(bead_id.clone());

                // Get neighbors
                let node_idx = match self.dag.node_map.get(&bead_id) {
                    Some(idx) => *idx,
                    None => return,
                };
                ...
                let neighbors: Vec<BeadId> = self
                    ...
                    .graph
                    .neighbors_directed(node_idx, Direction::Outgoing)
                    .filter_map(|idx| self.dag.graph.node_weight(idx).cloned())
                    .collect();

                // Consider successors of v
                for neighbor in neighbors {
                    let neighbor_index = self.indices.get(&neighbor).copied();
                    ...
                    match neighbor_index {
                        Some(_neighbor_idx) => {
                            ...
                            if self.on_stack.contains(&neighbor) {
                                ...
                                let current_low = self.low_links.get(&bead_id).copied().unwrap_or(usize::MAX);
                                let neighbor_low = self.low_links.get(&neighbor).copied().unwrap_or(usize::MAX);
                                if neighbor_low < current_low {
                                    self.low_links.insert(bead_id.clone(), neighbor_low);
                                }
                            }
                        }
                        None => {
                            // Successor w has not yet been visited; recurse on it
                            self.strongconnect(neighbor.clone());

                            // After returning from recursion, update low_link
                            let current_low = self.low_links.get(&bead_id).copied().unwrap_or(usize::MAX);
                            let neighbor_low = self.low_links.get(&neighbor).copied().unwrap_or(usize::MAX);
                            if neighbor_low < current_low {
                                self.low_links.insert(bead_id.clone(), neighbor_low);
                ...
                let current_index = self.indices.get(&bead_id).copied().unwrap_or(usize::MAX);
                let current_low = self.low_links.get(&bead_id).copied().unwrap_or(usize::MAX);
