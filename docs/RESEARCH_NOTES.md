...
- Keep gate execution moon-only in `src/main.rs`: `moon run :check|:test|:quick|:ci`; ship gate order is `:ci -> Git branch/PR verification -> :quick -> :test`.
- Keep workspace provisioning Git-only: use Git branches by default and Git worktrees only when physical isolation is required.
