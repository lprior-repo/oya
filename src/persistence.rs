use crate::domain::{
    AgentId, AgentState, AgentStatus, Artifact, BeadId, GateResult, Run, RunId, RunState,
    ShipDecision, StageAttempt, StageName, StageResult,
};
...
impl OyaDb {
    ...
    #[allow(dead_code)]
    pub async fn get_records_by_prefix<T: DeserializeOwned + Send + 'static>(
        &self,
        tree_name: &str,
        prefix: &[u8],
    ...
    pub async fn get_agent_state(&self, agent_id: &str) -> Result<Option<AgentState>, OyaDbError> {
        ...
        match record {
            Some(r) => {
                let current_stage: Option<StageName> = r
                    .current_stage
                    .map(|s| {
    ...
    pub async fn insert_bead_run(&self, run: &Run) -> Result<(), OyaDbError> {
    ...
    pub async fn get_run(&self, run_id: &str) -> Result<Option<Run>, OyaDbError> {
    ...
    pub async fn get_stage_results(&self, run_id: &str) -> Result<Vec<StageResult>, OyaDbError> {
        ...
        for r in records {
            let stage: StageName = serde_json::from_str(&r.stage)
                .map_err(|e| OyaDbError::Serialization(e.to_string()))?;
            let output = r
                .output
