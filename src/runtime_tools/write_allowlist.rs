...
fn validate_path_within_workspace(
    ...
    // Canonicalize would be ideal but requires filesystem access
    // Instead, we check if the path starts with workspace_root followed by
    // a separator (to handle sibling directories like project-other)
    let path_str = normalize_path(path);
    let root_str = normalize_path(workspace_root);

    // Path must be exactly the root, or start with root + separator
    let is_exact_match = path_str == root_str;
    let is_subpath = path_str.starts_with(&format!("{root_str}/"));

    if !is_exact_match && !is_subpath {
...
/// Normalizes a path for comparison (removes trailing slashes, etc.)
