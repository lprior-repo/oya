...
pub mod restate_handlers {
    use super::*;
    use crate::domain::{BeadId, Run, RunId, StageResult};
    use crate::infrastructure::persistence::{OyaDb, OyaDbError};
    use restate_sdk::prelude::*;

    /// Restate workflow: Start a new Run
    ///
    /// Durable execution: State persists in Restate, survives restarts
    ...
    pub async fn start_run_workflow(
        ...
        // Initialize run in Pending state
        let run = Run {
            id: run_id.clone(),
            bead_id,
    ...
    /// Restate workflow: Advance to next stage
    ///
    /// Durable: Validates transition, persists state, schedules next
    #[restate_sdk::workflow]
    pub async fn advance_to_next_stage(
        ...
        if let Some(next) = next_stage {
            validate_transition(&run.state, next)?;

            // Update state (side effect in shell)
            let new_state = RunState::Running { current_stage: next };
            ctx.run(|| async {
    ...
    /// Restate workflow: Handle stage failure with retry routing
    ///
    /// Durable: Schedules backoff, routes to retry lane
    #[restate_sdk::workflow]
    pub async fn handle_stage_failure(
        ...
        // Determine retry action (pure core)
        let action = determine_retry_action(failed_stage, attempt, &failure, &reason)?;

        match &action {
            RetryAction::Scheduled { backoff_duration, next_stage } => {
                // Durable sleep for backoff
    ...
    /// Restate workflow: Complete workflow with terminal state
    #[restate_sdk::workflow]
    pub async fn complete_workflow(
        ...
        let final_state = match terminal_state {
            ...
            TerminalState::Aborted => RunState::Aborted {
                reason: rationale.map_or_else(String::new, std::convert::identity),
    ...
    /// Restate workflow: Replay workflow from persisted state
    ///
    /// Idempotent: Returns current state, can be called multiple times
    #[restate_sdk::workflow]
