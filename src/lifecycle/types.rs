...
mod tests {
    ...
    #[test]
    fn test_bead_id_parse_too_long() {
        let id = "a".repeat(MAX_BEAD_ID_LEN + 1);
        assert_eq!(
            BeadId::parse(&id),
            Err(BeadIdError::TooLong { len: MAX_BEAD_ID_LEN + 1, max: MAX_BEAD_ID_LEN })
        );
    }
    ...
    fn test_lifecycle_error_classification() {
        let terminal = LifecycleError::terminal(FailureCategory::Workspace, "workspace failed");
        let transient = LifecycleError::transient(FailureCategory::Command, "timeout");

        assert!(terminal.is_terminal());
        assert_eq!(terminal.class(), FailureClass::Terminal);
        assert_eq!(terminal.category(), FailureCategory::Workspace);
        assert_eq!(terminal.message(), "workspace failed");

        assert!(!transient.is_terminal());
        assert_eq!(transient.class(), FailureClass::Transient);
        assert_eq!(transient.category(), FailureCategory::Command);
        assert_eq!(transient.message(), "timeout");
    }
}
