...
mod tests {
    ...
    async fn e2e_test_quality_gate_enforces_threshold() {
        ...
        if score < 70 {
            ...
            assert!(gate_result.is_err(), "Gate transition should fail for score < 70");
        } else {
            let gate_result = attempt_gate_transition(&bead, "planning").await;
            assert!(gate_result.is_ok(), "Gate transition should succeed for score >= 70");
        }
    }
    ...
    async fn e2e_test_planning_generates_lifecycle_steps() {
        ...
        assert!(step_names.contains(&"mark_in_progress"), "Should include mark_in_progress step");
        assert!(step_names.contains(&"workspace_add"), "Should include workspace_add step");
        assert!(step_names.contains(&"opencode"), "Should include opencode step");
        assert!(step_names.contains(&"moon_ci"), "Should include moon_ci step");
    }
    ...
    async fn e2e_test_full_pipeline_discovery_to_planning() {
        ...
        if score >= 70 {
            ...
            assert!(gate_result.is_ok(), "Gate transition should succeed for valid score");
    ...
    async fn execute_failing_lifecycle(_bead: &BeadData) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn get_quality_score_color(
        _score: u32,
