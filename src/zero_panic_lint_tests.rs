...
mod zero_panic_lint_tests {
    ...
    fn test_main_has_unsafe_code_forbid() {
        ...
        if let Ok(content) = fs::read_to_string(main_path) {
            assert!(
                ...
                "main.rs must forbid unsafe code"
            );
        } else {
            ...
            if let Ok(content) = fs::read_to_string(&file) {
                for (line_num, line) in content.lines().enumerate() {
                    ...
                    if trimmed.contains(".unwrap()") && !trimmed.starts_with("//") {
                        violations.push(format!("{}:{}: found .unwrap() - {}", file, line_num + 1, trimmed));
                    }
                }
            }
        }
            ...
            if let Ok(content) = fs::read_to_string(&file) {
                for (line_num, line) in content.lines().enumerate() {
                    ...
                    if trimmed.contains(".expect(") && !trimmed.starts_with("//") {
                        violations.push(format!("{}:{}: found .expect() - {}", file, line_num + 1, trimmed));
                    }
                }
            }
        }
            ...
            if let Ok(content) = fs::read_to_string(&file) {
                for (line_num, line) in content.lines().enumerate() {
                    ...
                    if (trimmed.contains("panic!(") || trimmed.contains("todo!(") || trimmed.contains("unimplemented!(")) 
                        && !trimmed.starts_with("//") {
                        violations.push(format!("{}:{}: found panic-like macro - {}", file, line_num + 1, trimmed));
                    }
                }
            }
        }
            ...
            if Path::new(file).exists() {
                if let Ok(content) = fs::read_to_string(file) {
                    assert!(
                        ...
                        "{} must have #![deny(clippy::unwrap_used)]", file
                    );
                    assert!(
                        has_lint_attribute(&content, "expect_used"),
                        "{} must have #![deny(clippy::expect_used)]", file
                    );
                    assert!(
                        has_lint_attribute(&content, "panic"),
                        "{} must have #![deny(clippy::panic)]", file
                    );
                }
            }
        }
    ...
    fn test_lattice_module_has_lints() {
        let lattice_files = vec![
            "src/lattice/mod.rs",
            "src/lattice/coordinator.rs",
            "src/lattice/execution.rs",
        ];
    ...
    fn test_intent_module_has_lints() {
        let intent_files = vec![
            "src/intent/mod.rs",
            "src/intent/parser.rs",
            "src/intent/executor.rs",
        ];
    ...
    fn test_clippy_config_exists() {
        assert!(
            ...
            "Project must have .clippy.toml configuration"
        );
    }
                    ...
                    for line in content.lines() {
                        ...
                        if (trimmed.starts_with("pub fn ") || trimmed.starts_with("pub async fn ")) 
                            ...
                            if trimmed.contains("-> ") && !trimmed.contains("()") {
                                warnings.push(format!("{}: consider using Result for fallible function", file));
                            }
                        }
                    }
                }
