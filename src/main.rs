...
async fn pick_repo_slug(explicit: Option<String>, jj_repo: Option<&str>) -> anyhow::Result<String> {
    if let Some(value) = explicit {
        return parse_repo_slug(&value).map_err(anyhow::Error::msg);
    }
...
fn ensure_repo_matches_jj_origin(selected: &str, jj_repo: Option<&str>) -> anyhow::Result<()> {
...
async fn ensure_repo_exists(repo: &str) -> anyhow::Result<()> {
    let output = TokioCommand::new("gh")
        ...
        .output()
        .await
        .map_err(|error| anyhow::anyhow!("failed to run gh repo view for `{repo}`: {error}"))?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(anyhow::anyhow!("repo `{repo}` is not accessible: {}", stderr.trim()));
    }
    let stdout = String::from_utf8(output.stdout)
        .map_err(|error| anyhow::anyhow!("gh output was not UTF-8: {error}"))?;
    let resolved = extract_repo_slug_from_gh_output(&stdout)?;
    if resolved != repo {
        return Err(anyhow::anyhow!("gh resolved repo `{resolved}` but expected `{repo}`"));
    }
    Ok(())
}
...
fn extract_repo_slug_from_jj_remote_output(raw: &str) -> anyhow::Result<Option<String>> {
...
fn parse_repo_slug_from_remote_url(value: &str) -> anyhow::Result<String> {
    ...
    parse_repo_slug(normalized).map_err(anyhow::Error::msg)
}
