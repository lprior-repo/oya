...
mod tests {
    ...
    fn test_score_parity_across_phases() {
        ...
        assert_eq!(answer_score.total, spec_score.total, 
            "Total scores must be equal across phases");
        assert_eq!(answer_score.completeness, spec_score.completeness,
            "Completeness scores must be equal");
        assert_eq!(answer_score.clarity, spec_score.clarity,
            "Clarity scores must be equal");
        assert_eq!(answer_score.correctness, spec_score.correctness,
            "Correctness scores must be equal");
        assert_eq!(answer_score.relevance, spec_score.relevance,
            "Relevance scores must be equal");
    }
    ...
    fn test_divergence_rejection() {
        ...
        if report.divergence.is_some() {
            assert!(!report.is_consistent, 
                "Audit must fail when divergence detected");
            assert!(!report.errors.is_empty(),
                "Divergence must produce error messages");
            
            let has_divergence_error = report.errors.iter().any(|e| {
                matches!(e, MathConsistencyError::ScoreDivergence { .. })
            });
            assert!(has_divergence_error,
                "Must include ScoreDivergence error when scores differ");
        }
    }
    ...
    fn test_weight_consistency_across_evaluators() {
        ...
        assert_eq!(answer_weights.completeness_weight, spec_weights.completeness_weight,
            "Completeness weights must match");
        assert_eq!(answer_weights.clarity_weight, spec_weights.clarity_weight,
            "Clarity weights must match");
        assert_eq!(answer_weights.correctness_weight, spec_weights.correctness_weight,
            "Correctness weights must match");
        assert_eq!(answer_weights.relevance_weight, spec_weights.relevance_weight,
    ...
    fn test_threshold_consistency_across_evaluators() {
        ...
        assert_eq!(answer_thresholds.minimum_total, spec_thresholds.minimum_total,
            "Minimum total thresholds must match");
        assert_eq!(answer_thresholds.maximum_total, spec_thresholds.maximum_total,
            "Maximum total thresholds must match");
        assert_eq!(answer_thresholds.passing_score, spec_thresholds.passing_score,
            "Passing score thresholds must match");
        assert_eq!(answer_thresholds.divergence_tolerance, spec_thresholds.divergence_tolerance,
    ...
    fn test_audit_report_structure() {
        ...
        assert!(!report.answer_score.total.is_nan(),
            "Answer score total must be valid number");
        assert!(!report.spec_score.total.is_nan(),
            "Spec score total must be valid number");
        
        if report.is_consistent {
            assert!(report.divergence.is_none() || report.divergence == Some(0.0),
                "Consistent audit should have zero or no divergence");
            assert!(report.errors.is_empty(),
                "Consistent audit should have no errors");
        } else {
            assert!(report.divergence.is_some(),
                "Inconsistent audit must report divergence");
            assert!(!report.errors.is_empty(),
                "Inconsistent audit must have errors");
        }
    }
    ...
    fn test_score_bounds_validation() {
        ...
        if let Ok(answer_score) = answer_result {
            assert!(answer_score.completeness >= 0.0 && answer_score.completeness <= 1.0,
                "Completeness score must be in [0, 1]");
            assert!(answer_score.clarity >= 0.0 && answer_score.clarity <= 1.0,
                "Clarity score must be in [0, 1]");
            assert!(answer_score.correctness >= 0.0 && answer_score.correctness <= 1.0,
                "Correctness score must be in [0, 1]");
            assert!(answer_score.relevance >= 0.0 && answer_score.relevance <= 1.0,
                "Relevance score must be in [0, 1]");
            assert!(answer_score.total >= 0.0,
                "Total score must be non-negative");
        }
        
        if let Ok(spec_score) = spec_result {
            assert!(spec_score.completeness >= 0.0 && spec_score.completeness <= 1.0,
                "Completeness score must be in [0, 1]");
            assert!(spec_score.clarity >= 0.0 && spec_score.clarity <= 1.0,
                "Clarity score must be in [0, 1]");
            assert!(spec_score.correctness >= 0.0 && spec_score.correctness <= 1.0,
                "Correctness score must be in [0, 1]");
            assert!(spec_score.relevance >= 0.0 && spec_score.relevance <= 1.0,
                "Relevance score must be in [0, 1]");
            assert!(spec_score.total >= 0.0,
                "Total score must be non-negative");
        }
    }
    ...
    fn test_total_calculation_accuracy() {
        ...
        if let (Ok(answer_score), Ok(spec_score)) = (answer_result, spec_result) {
            ...
            let expected_answer_total = 
                answer_score.completeness * answer_weights.completeness_weight +
            ...
            let expected_spec_total = 
                spec_score.completeness * spec_weights.completeness_weight +
    ...
    fn test_multiple_identical_invocations_produce_identical_results() {
        ...
        assert_eq!(report1.is_consistent, report2.is_consistent,
            "Multiple invocations must have consistent consistency status");
        assert_eq!(report2.is_consistent, report3.is_consistent,
            "Multiple invocations must have consistent consistency status");
        
        assert_eq!(report1.answer_score, report2.answer_score,
            "Multiple invocations must produce identical Answer scores");
        assert_eq!(report2.answer_score, report3.answer_score,
            "Multiple invocations must produce identical Answer scores");
        
        assert_eq!(report1.spec_score, report2.spec_score,
            "Multiple invocations must produce identical Spec scores");
        assert_eq!(report2.spec_score, report3.spec_score,
            "Multiple invocations must produce identical Spec scores");
    }
}
